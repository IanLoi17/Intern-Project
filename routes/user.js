...
router.post('/productDetails/:id', (req, res) => {
    let id = req.params.id;
    ...
    if (qty >= 0) {
        Product.update({
            ...
            alertMessage(res, "success", "You have purchase the product successfully", "fa fa-check", true);
            res.redirect('/user/Product');
        }).catch(err => console.log(err));
    }

    else {
        alertMessage(res, "danger", "This product is currently out of stock", "fa fa-warning", true);
        res.redirect('/user/product');
    }
});
